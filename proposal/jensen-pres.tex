...
\addbibresource{/home/ajensen123@ad.ufl.edu/repo/lit-review/src/myBib.bib}
...
\end{columns}
\end{frame}
\begin{frame}[label={sec:org6816975}]{Our Proposition}
\begin{columns}
\begin{column}{0.5\columnwidth}
...
\end{columns}
\end{frame}
\begin{frame}[label={sec:org634a3cd}]{Constraints}
\begin{columns}
...
\end{columns}
\end{frame}
\begin{frame}[label={sec:org40dede6}]{Background - Projective Geometry}
\begin{center}
\includegraphics[width=0.8\textwidth]{/home/ajensen123@ad.ufl.edu/repo/lit-review/figures/raster/perspective-projection.png}
\end{center}
\end{frame}
\begin{frame}[label={sec:org1209a8d}]{Background - Model-Image Registration}
\begin{columns}
\begin{column}{0.5\columnwidth}
...
\end{columns}
\end{frame}
\begin{frame}[label={sec:orged4bd8f}]{Background - Model-Image Registration}
\begin{columns}
\begin{column}{0.5\columnwidth}
If we know the projective parameters of the fluoroscopy machine, can we tinker with \(T^{cam}_{implant}\) so that our virtual projection matches the fluoroscopic image?
\end{column}
...
\end{columns}
\end{frame}
\begin{frame}[label={sec:org632b7e7}]{Historical Overview}
...
\end{itemize}
\end{frame}
\begin{frame}[label={sec:orge0cee54}]{Pre-Computed Projections}
\begin{columns}
\begin{column}{0.5\columnwidth}
\begin{itemize}
...
\end{columns}
\end{frame}
\begin{frame}[label={sec:org5c7cefb}]{Limitations of Pre-Computed Projections}
\begin{columns}
\begin{column}{0.5\columnwidth}
\begin{itemize}
...
\begin{frame}[label={sec:org2627ceb}]{Motion Capture (MoCap)}
\begin{columns}
\begin{column}{0.5\columnwidth}
\begin{itemize}
...
\end{columns}
\end{frame}
\begin{frame}[label={sec:org1d03c66}]{Limitations of Motion Capture}
\begin{columns}
\begin{column}{0.5\columnwidth}
Skin Mounted
\begin{itemize}
...
\begin{frame}[label={sec:orgf29baf7}]{Biplane Imaging}
\begin{columns}
\begin{column}{0.5\columnwidth}
\begin{itemize}
...
\end{columns}
\end{frame}
\begin{frame}[label={sec:orgf52d3ca}]{Limitations of Biplane Imaging}
\begin{columns}
\begin{column}{0.5\columnwidth}
\begin{itemize}
...
\begin{frame}[label={sec:org4e3496e}]{Iterative Projections}
\begin{columns}
...
\end{columns}
\end{frame}
\begin{frame}[label={sec:org4c88496}]{Limitations of (historic) Iterative Projection Methods}
\begin{columns}
\begin{column}{0.54\columnwidth}
\begin{itemize}
...
\begin{frame}[label={sec:orge0b1f5b}]{Roentgen Stereophotogrammetry (RSA)}
\begin{columns}
\begin{column}{0.5\columnwidth}
\begin{itemize}
...
\end{columns}
\end{frame}
\begin{frame}[label={sec:orgdb5d118}]{Limitations of RSA}
\begin{columns}
\begin{column}{0.5\columnwidth}
\begin{itemize}
...
\section{Aims}
\label{sec:orgad53a03}
\begin{frame}[label={sec:org2814380}]{Aims}
\begin{columns}
...
\label{sec:orgf043b68}
\begin{frame}[label={sec:orgacfbdaa}]{Goal}
Demonstrate the feasibility of a fully autonomous, model-image registration pipeline.
\end{frame}
\begin{frame}[label={sec:org808e717}]{Method}
\begin{itemize}
...
\end{center}
\end{frame}
\begin{frame}[label={sec:org717f303}]{Autonomous Implant Detection Using Convolutional Neural Networks}
\begin{columns}
\begin{column}{0.5\columnwidth}
\begin{itemize}
\item 2 CNNs
...
\end{columns}
\end{frame}
\begin{frame}[label={sec:org1f49f77}]{Neural Network Data}
\begin{columns}
\begin{column}{0.5\columnwidth}
\begin{itemize}
...
\end{columns}
\end{frame}
\begin{frame}[label={sec:org4c09f05}]{Neural Network Robustness}
\begin{itemize}
...
\end{center}
\end{frame}
\begin{frame}[label={sec:org3cc9658}]{Normalized Fourier Descriptor Shape Libraries}
\begin{columns}
...
\end{columns}
\end{frame}
\begin{frame}[label={sec:org5d31683}]{Pose Refinement Using Global Optimization}
\begin{columns}
\begin{column}{0.5\columnwidth}
\begin{itemize}
...
\begin{frame}[label={sec:orga92dc3c}]{Contour-based Objective Function}
\begin{columns}
\begin{column}{0.5\columnwidth}
\begin{itemize}
...
\end{columns}
\end{frame}
\begin{frame}[label={sec:org13b8fe8}]{Improving Robustness}
\begin{columns}
\begin{column}{0.5\columnwidth}
\begin{itemize}
...
\end{columns}
\end{frame}
\begin{frame}[label={sec:org9063002}]{Optimization Routine}
\begin{itemize}
...
\begin{frame}[label={sec:org74d6e75}]{Lipschitzian Optimization}
\begin{columns}
\begin{column}{0.5\columnwidth}
\begin{itemize}
...
\begin{frame}[label={sec:org1808790}]{Lipschitzian Optimization without the Lipschitz Constant}
\begin{center}
...
\end{center}
\end{frame}
\begin{frame}[label={sec:org2963c7d}]{Trisecting Region}
\begin{columns}
...
\end{columns}
\end{frame}
\begin{frame}[label={sec:orge6594d8}]{Another Iteration}
\begin{columns}
\begin{column}{0.4\columnwidth}
\begin{equation*}
  \begin{bmatrix}
...
\begin{frame}[label={sec:org8fdd836}]{Determining Potentially Optimal Regions}
\begin{itemize}
...
\end{center}
\end{frame}
\begin{frame}[label={sec:orgac12409}]{DiRECT for Joint Track Machine Learning}
\begin{itemize}
...
\begin{center}
\begin{tabular}{lllr}
Stage & Budget [Iterations] & Search Range [mm,deg] & Dilation (pixels)\\[0pt]
\hline
``Tree'' & \textasciitilde{}20,000 & \(\pm 45\) & 5\\[0pt]
``Branch'' & \textasciitilde{}20,000 & \(\pm 25\) & 3\\[0pt]
``Leaf'' & \textasciitilde{}10,000 & \(\pm 100\) \((z_{trans})\) / \(\pm 3\) \((else)\) & 1\\[0pt]
\end{tabular}
\end{center}
\end{frame}
\begin{frame}[label={sec:org133a484}]{Testing Performance}
Now that we have our refined poses, how well does out system perform?
\begin{center}
\includegraphics[width=\textwidth]{/home/ajensen123@ad.ufl.edu/repo/lit-review/figures/raster/jtml-pipeline.png}
\end{center}
\end{frame}
\begin{frame}[label={sec:org2f54d37}]{Validation}
\begin{itemize}
...
\end{center}
\end{frame}
\begin{frame}[label={sec:orga8c11e9}]{Awards}
...
\label{sec:orge226917}
\begin{frame}[label={sec:org0b454ac}]{Goal}
\begin{itemize}
...
\begin{frame}[label={sec:orga6ae114}]{Translation}
\begin{itemize}
...
\begin{frame}[label={sec:org672412c}]{Symmetry Traps}
...
\end{center}
\end{frame}
\begin{frame}[label={sec:orgc970190}]{Solving the Symmetric Pose}
\begin{columns}
\begin{column}{0.5\columnwidth}
...
\end{columns}
\end{frame}
\begin{frame}[label={sec:orgf4d3bf6}]{Four Approaches}
\begin{itemize}
...
\begin{frame}[label={sec:org6a65599}]{Virtual Ligaments}
\begin{equation*}
...
\begin{frame}[label={sec:orgb3b338e}]{Binary Selection}
\begin{enumerate}
...
\end{frame}
\begin{frame}[label={sec:orgcae305e}]{Bland-Altmann Calibration Constant}
\begin{itemize}
...
\begin{frame}[label={sec:orga60ebd6}]{Fully Connected Network}
\begin{columns}
\begin{column}{0.5\columnwidth}
\begin{itemize}
...
\begin{frame}[label={sec:org313e26f}]{Timeline}
\begin{itemize}
...
\label{sec:org3141370}
\begin{frame}[label={sec:org7791641}]{Goal}
...
\begin{frame}[label={sec:org99f4dcb}]{Methods}
\begin{itemize}
\item 20-30 patients
...
\label{sec:orgafd692c}
\begin{frame}[label={sec:org6622bbb}]{Goal}
...
\end{frame}
\begin{frame}[label={sec:orgbad799e}]{Motivation}
\begin{itemize}
...
\begin{frame}[label={sec:org2b0f84f}]{Method}
\begin{itemize}
...
\label{sec:org409fffa}
\begin{frame}[label={sec:org03e1ff3}]{Joint Track Auto Toolkit (JTAT)}
...
\end{frame}
\begin{frame}[label={sec:org901a486},fragile, allowframebreaks, label=]{Presentations}
\begin{refsection}
  \input{nocites-pres}
...
\begin{frame}[label={sec:org85584dc},fragile, allowframebreaks, label=]{Publications}
\begin{refsection}
  \input{nocites-pubs}
...
\begin{frame}[label={sec:orgeb97b1a}]{Timeline}
\begin{center}
\begin{tabular}{ll}
Date(s) & Event\\[0pt]
\hline
2015-2019 & Mech. Eng. B.S, Magna Cum Laude, UF\\[0pt]
April 2019 - April 2020 & Internship at Exactech\\[0pt]
April 2020 & Started in Miller Lab\\[0pt]
August 2020 & Officially Started PhD at UF\\[0pt]
November 2021 & Best Presentation Award at ISTA: Emerging Technologies\\[0pt]
April 2022 & Submitted JTML for HAP Paul Award\\[0pt]
September 2022 & HAP Paul Award at ISTA 2022\\[0pt]
\hline
June 2023 & Single-plane limitations paper submitted\\[0pt]
July 2023 & Est. IRB Approval\\[0pt]
August 2023 & v1.0 JTAT\\[0pt]
December 2023 \textasciitilde{} May 2024 & Patient Data Fully Collected (Aims 3/4)\\[0pt]
August 2024 & Papers for Aims 3/4 Submitted\\[0pt]
December 2024 - April 2025 & Est. Graduation\\[0pt]
\end{tabular}
\end{center}
\end{frame}
\begin{frame}[label={sec:org8234962},standout]{Thank you!}
Thanks for listening!!
\end{frame}
\section{References}
\label{sec:orgcd342d0}
\begin{frame}[label={sec:org7b98550},fragile, allowframebreaks, label=]{References}
